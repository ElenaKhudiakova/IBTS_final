...
// true - отвечает за то, что процесс будет запущен по команде ProcessThreadPtr->Start()
// Iterator->GetFileSystem()->GetClusterSize() - передаем размер кластера
// Form1 - для связи с формой
// useSignatureSearch  - отвечаем за вопрос, нужно ли искать сигнатуры
//---------------------------------------------------------------------------
void __fastcall ReadThread::Execute()
{
	//---- Place thread code here ----
	Synchronize(&OnStart);
	ProcessThreadPtr->Start(); // запускаем поток, который будет обрабатывать данные

	// сначала первое состояние - переход в режим готовности, при этом уже считан первый кластер в ProcessThread
	...
	for (Iterator->First(); !Iterator->IsDone(); Iterator->Next()) {
		...
		ProcessThreadPtr->clusterNumber = Iterator->GetCurrentPosition(); // clusterNumber - текущий кластер
	...
	ProcessThreadPtr->Terminate();   // завершаем поток процесса

	Synchronize(&OnStop);
	ProcessThreadPtr->WaitFor();  // ждем, пока ProcessThread не завершится (лекция)

	delete ProcessThreadPtr;   // очистка указателя на поток (лекция)
}
//---------------------------------------------------------------------------
