...
// »спользуем bool как логический тип данных. ѕеременные этого типа могут содержать
// всего два значени€ Ц true (истина=1) и false (ложь=0).
...
char* GetSignatureName(ClusterSignature clusterSignature) {
	...
	case ClusterSignature::PNG:  // »спользуем глобальные переменные, то есть глобальный оператор разрешени€ (::)
		return "PNG";
		break;
 ...
 //  онструкци€ switch-case Ч это удобна€ замена длинной if-else конструкции,
 // котора€ сравнивает переменную с несколькими константными значени€ми.
